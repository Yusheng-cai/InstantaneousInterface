...
/* Subroutine */ int dicfs_(int *n, int *nnz, double *a,
                            ...
                            wa1, double *wa2)
{
    ...
    /*     Deterstd::min<double>e a lower bound for the step. */
    if (*alpha <= 0.)
    {
        alphas = .001;
    }
    ...
    *alpha = 0.;
    ...
    for (i__ = 1; i__ <= i__1; ++i__)
    {
        ...
        else
        {
            ...
            /* Computing 2nd power */
            d__3 = wa2[i__];
            d__1 = *alpha, d__2 = -adiag[i__] * (d__3 * d__3);
            *alpha = std::max<double>(d__1,d__2);
        }
    }
    if (*alpha > 0.)
    {
    ...
    /*     the lower bound alphas until we deterstd::min<double>e a lower bound that */
    /*     is not acceptable. We then increase the shift. */
    /*     The lower bound is decreased by nbfactor at most nbstd::max<double> times. */
    nb = 1;
    while (1)
    {
        ...
        for (i__ = 1; i__ <= i__1; ++i__)
        {
            lcol_ptr__[i__] = acol_ptr__[i__];
        }
        i__1 = *nnz;
        for (i__ = 1; i__ <= i__1; ++i__)
        {
            lrow_ind__[i__] = arow_ind__[i__];
        }
        ...
        for (j = 1; j <= i__1; ++j)
        {
            /* Computing 2nd power */
            d__1 = wa2[j];
            ldiag[j] = adiag[j] * (d__1 * d__1) + *alpha;
        }
        i__1 = *n;
        for (j = 1; j <= i__1; ++j)
        {
          ...
          double *w)
{
    ...
    *info = 0;
    ...
    for (j = 1; j <= i__1; ++j)
    {
        ...
        if (nlj >= 1)
        {
            /*           Deterstd::min<double>e the kth smallest elements in the current */
...
/* Subroutine */
int dsel2_(int *n, double *x, int *keys, int
           *k)
{
    /* System generated locals */
    int i__1;
    ...
    while (l < u)
    {
        ...
        if ((d__1 = x[keys[l]], std::fabs(d__1)) > (d__2 = x[keys[p1]], std::fabs(d__2)))
        {
            swap = keys[l];
            keys[l] = keys[p1];
            keys[p1] = swap;
        ...
        if ((d__1 = x[keys[p2]], std::fabs(d__1)) > (d__2 = x[keys[p3]], std::fabs(d__2)))
        {
            swap = keys[p2];
            keys[p2] = keys[p3];
            keys[p3] = swap;
        ...
        if ((d__1 = x[keys[p3]], std::fabs(d__1)) > (d__2 = x[keys[p1]], std::fabs(d__2)))
        {
            swap = keys[p3];
            keys[p3] = keys[u];
            keys[u] = swap;
            if ((d__1 = x[keys[p2]], std::fabs(d__1)) > (d__2 = x[keys[p3]], std::fabs(
                d__2)))
            {
                swap = keys[p2];
                keys[p2] = keys[p3];
        ...
        else
        {
            ...
            keys[p1] = keys[u];
            keys[u] = swap;
            if ((d__1 = x[keys[l]], std::fabs(d__1)) > (d__2 = x[keys[p1]], std::fabs(
                d__2)))
            {
                swap = keys[l];
                keys[l] = keys[p1];
        ...
        if ((d__1 = x[keys[p1]], std::fabs(d__1)) > (d__2 = x[keys[p3]], std::fabs(d__2)))
        {
            if ((d__1 = x[keys[l]], std::fabs(d__1)) <= (d__2 = x[keys[p3]], std::fabs(
                d__2)))
            {
                swap = keys[l];
                keys[l] = keys[p3];
        ...
        {
            if ((d__1 = x[keys[p2]], std::fabs(d__1)) <= (d__2 = x[keys[p1]], std::fabs(
                d__2)))
            {
                swap = keys[l];
                keys[l] = keys[p1];
        ...
        {
            if ((d__1 = x[keys[i__]], std::fabs(d__1)) < (d__2 = x[keys[l]], std::fabs(
                d__2)))
            {
                ++m;
                swap = keys[m];
        ...
        if ((u - l) * 3 > lp << 1 && *k > m)
        {
            ...
            {
                if ((d__1 = x[keys[i__]], std::fabs(d__1)) == (d__2 = x[keys[m]],
                    std::fabs(d__2)))
                {
                    ++p;
                    swap = keys[p];
