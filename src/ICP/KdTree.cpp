...
void gs::KdTree::build(std::vector<Point*> &pointCloud, int start, int end, int sortOn)
{
	...
	int mid = (int)std::floor((float)numPoints*0.5) + start;
...
void gs::KdTree::radiusSearch(Point* p, float* radius, Point* result)
{
	if (isLeaf())
	{
		float d = std::sqrt(std::pow(__node->pos[0] - p->pos[0], 2.0) + pow(__node->pos[1] - p->pos[1], 2.0) + pow(__node->pos[2] - p->pos[2], 2.0));
		if (d < *radius)
		{
			*radius = d;
...
void gs::KdTree::search(Point* p, Point* result)
{
	...
	float radius = std::sqrt(std::pow(p->pos[0] - result->pos[0], 2.0) + std::pow(p->pos[1] - result->pos[1], 2.0) + std::pow(p->pos[2] - result->pos[2], 2.0));
...
void gs::KdTree::mergeSort(std::vector<Point*> &pointCloud, int start, int end)
{
	...
	if (start > end)
	{
		mid = (int)std::floor((end + start)*0.5);
